            ...
            if (typeof info.status != 'undefined' && info.status == 'top' || info.status == 'mid') {
                controller.storage.users.get(message.user, function (error, output) {
                    if (!output) {
                        ...
                        results = getMonthlyOutput(output, info.status);
                        if (results == 404) {
                ...
                bot.reply(message, "My apologies - Monthly reporting is only available for paying subscribers! You can check out plans on your site: https://getinternal.co");
            }
        });
    });
}
